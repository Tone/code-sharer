...
async function interactive(dir: string) {
  ...
  const materialName = path.dirname(dir)
  ...
  const question: QuestionCollection = [
    {
      ...
      filter: ({ packages }) => packages.split(' ').map((p: string) => p.split('@')),
      require: false
    }
  ]
...
async function submit(srcDir: string, repository: Repository, config: MaterialConfig) {
